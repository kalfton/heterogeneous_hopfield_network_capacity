...
sys.argv += "--W_notsymmetric --n_neuron 150 --n_neuron_L2 450 --n_pattern 500 --perc_active_L1 0.5 --perc_active_L2 0.01 \
    --network_type RBM --method svm --neuron_base_state -1 --use_reg --kappa 0.5".split()
...
for paras in parameter_sets:
    ...
    # # plot the histogram of W_ij*W_ji:
    # plt.sca(axes[0,2])
    # bins = np.arange(-10,10,0.2)
    # plt.hist((W_12_trained*W_21_trained.mT).detach().numpy().flatten(), density=True, bins = bins, alpha=0.5, color=colorsets[parameter_sets.index(paras)])
