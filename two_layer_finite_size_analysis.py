...
def caculate_numerical_cacpacity(pars, max_n_pattern:int, step:int, epsilon, seed1, seed2):
    ...
    # patterns_L1_unique = torch.unique(patterns_L1, dim=0)
    # patterns_L2_unique = torch.unique(patterns_L2, dim=0)
    # n_pattern_unique = min(patterns_L1_unique.shape[0], patterns_L2_unique.shape[0]) # the number of unique patterns is the minimum of the two layers
    # patterns_L1 = patterns_L1_unique[:n_pattern_unique,:]
    # patterns_L2 = patterns_L2_unique[:n_pattern_unique,:]
...
if __name__ == '__main__':
    ...
    sys.argv += "--W_notsymmetric --n_neuron 150 --n_neuron_L2 450 --network_type RBM --method svm --training_max_iter 1000 \
        --neuron_base_state -1 --epsilon 0.01 --n_repeat 50 --n_process 10".split() #--use_kappa --kappa 0.5
    ...
    for iii in range(len(n_neuron_L1_set)):
        ...
        with open('results/numerical_two_layer_finite_size_'+'k = %.1f, n_layer1 = %d, n_layer_2 = %d, repeat = %d' % (kappa, n_neuron, n_neuron_L2, n_repeat) + utils.make_name_with_time() +'.pkl', 'wb') as f:
            pickle.dump([capacities, information_capacity, perc_act_L1_set, perc_act_L2_set ,pars], f)
        print("end")

        # get the arguments of the maximum capacity:
    ...
    # capacities_matrix = np.zeros((len(n_neuron_L1_set), len(perc_act_L2_set), len(perc_act_L1_set)))
    # for iii in range(len(n_neuron_L1_set)):
    #     with open('results/numerical_two_layer_temp_'+'k = %.1f, n_layer1 = %d, n_layer_2 = %d' % (kappa, n_neuron_L1_set[iii], n_neuron_L2_set[iii]) +'.pkl', 'rb') as f:
    #         capacities, information_capacity, perc_act_L1_set, perc_act_L2_set ,pars = pickle.load(f)
    #         capacities_matrix[iii,:,:] = capacities
    #         i_max, j_max = np.unravel_index(np.argmax(capacities, axis=None), capacities.shape)
    #         arg_max_perc_act_L1[iii] = perc_act_L1_set[j_max]
    #         arg_max_perc_act_L2[iii] = perc_act_L2_set[i_max]
    ...
    for i in range(len(n_neuron_L1_set)):
        ...
        threshold = max_capacities[i]*0.90
        ...
        # plt.contour(xx, yy, capacity_matrix_binary, levels=[-0.5, 0.5, 1.5],colors=[colors[i]],  alpha=0.5, linewidths=2, linestyles = 'dashed')
        # plot the maximum capacity point:
        plt.scatter(arg_max_perc_act_L1[i], arg_max_perc_act_L2[i], s=10, color=colors[i])
        plt.text(arg_max_perc_act_L1[i], arg_max_perc_act_L2[i], str(n_neuron_L1_set[i]*4), fontsize=12, ha='right', va='bottom')
