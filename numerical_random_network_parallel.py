...
def cacpacity_for_parallel(pars_copy, seed1, seed2):
    ...
    training_max_iter = pars_copy["training_max_iter"]
    method = pars_copy["method"]
    lr_PLA = pars_copy["lr_PLA"]
    ...
    cap = calculate_numerical_cacpacity(pars_copy, n_neuron*7, 5, epsilon=epsilon, seed1=seed1, seed2=seed2, neuron_act_prop=neuron_act_prop, network=network)
    ...
    print("Ratio_coon_std, %f, pattern_act_std, %f, The theoretical capacity is: %f" % (pars_copy["ratio_conn_std"], pars_copy["pattern_act_std"], capacity))
...
if __name__ == '__main__':
    ...
    # sys.argv += "--W_notsymmetric --n_neuron 100 --method svm --change_in_degree --n_repeat 10 --n_process 10  --pattern_act_mean 0.25".split() # --kappa 0.5 --use_reg
        ...
        for i in range(len(ratio_conn_std_list)):
            for j in range(len(pattern_act_std_list)):
                for k in range(n_repeat):
                    ...
                    # numerical capacity
                    seed1_pool = np.random.randint(100000, size=n_repeat)
                    seed2_pool = np.random.randint(100000, size=n_repeat)
                    pars_copy = pars.copy()
                    ...
                    pool_results_object[i][j].append(pool.apply_async(cacpacity_for_parallel, args=(pars_copy, seed1_pool[k], seed2_pool[k])))
    ...
    pool.close()
    pool.join()
    # # debug the parallel computation by using for loop:
    # for i in range(len(ratio_conn_std_list)):
    #     for j in range(len(pattern_act_std_list)):
    #         for k in range(n_repeat):
    #             print("ratio_conn_std: ", ratio_conn_std_list[i], "pattern_act_std: ", pattern_act_std_list[j])
    ...
    #             pars_copy["ratio_conn_std"] = ratio_conn_std_list[i]
    #             pars_copy["pattern_act_std"] = pattern_act_std_list[j]
    #             capacities[i,j,k], capacities_theoretical[i,j,k], capacities_matched[i,j,k], capacities_theoretical_matched[i,j,k], capacities_anti_matched[i,j,k], capacities_theoretical_anti_matched[i,j,k] = cacpacity_for_parallel(pars_copy, seed1, seed2)


    # save the results:
    with open('results/Heterogeneous_network_capacity_k=%d_input_heter_=%d' % (pars["kappa"], pars["change_in_degree"]) + utils.make_name_with_time() + '.pkl', 'wb') as f:
    ...
    h = plt.contourf(xx, yy, capacities_mean, cmap='viridis', vmin=vmin, vmax=vmax,levels = np.arange(vmin, vmax, 0.1))
    # label the colorbar:
    cbar = plt.colorbar()
    ...
    # plot the meshgrid of capacity matrix:
    xx, yy = np.meshgrid(pattern_act_std_list, ratio_conn_std_list)
    h = plt.contourf(xx, yy, capacities_theoretical_mean, cmap='viridis', vmin=vmin, vmax=vmax,levels = np.arange(vmin, vmax, 0.1))
    # label the colorbar:
    cbar = plt.colorbar()
    cbar.set_label("Capacity", rotation=270, labelpad=20)
    plt.xlabel("Neuron activation heterogeneity")
    ...
    # plot the meshgrid of capacity matrix:
    xx, yy = np.meshgrid(pattern_act_std_list, ratio_conn_std_list)
    h = plt.contourf(xx, yy, capacities_matched_mean, cmap='viridis', vmin=vmin, vmax=vmax,levels = np.arange(vmin, vmax, 0.1))
    # label the colorbar:
    cbar = plt.colorbar()
    cbar.set_label("Capacity", rotation=270, labelpad=20)
    plt.xlabel("Neuron activation heterogeneity")
    ...
    # plot the meshgrid of capacity matrix:
    xx, yy = np.meshgrid(pattern_act_std_list, ratio_conn_std_list)
    h = plt.contourf(xx, yy, capacities_theoretical_matched_mean, cmap='viridis', vmin=vmin, vmax=vmax,levels = np.arange(vmin, vmax, 0.1))
    # label the colorbar:
    cbar = plt.colorbar()
    cbar.set_label("Capacity", rotation=270, labelpad=20)
    plt.xlabel("Neuron activation heterogeneity")
    ...
    # plot the meshgrid of capacity matrix:
    xx, yy = np.meshgrid(pattern_act_std_list, ratio_conn_std_list)
    h = plt.contourf(xx, yy, capacities_anti_matched_mean, cmap='viridis', vmin=vmin, vmax=vmax,levels = np.arange(vmin, vmax, 0.1))
    # label the colorbar:
    cbar = plt.colorbar()
    cbar.set_label("Capacity", rotation=270, labelpad=20)
    plt.xlabel("Neuron activation heterogeneity")
    ...
    # plot the meshgrid of capacity matrix:
    xx, yy = np.meshgrid(pattern_act_std_list, ratio_conn_std_list)
    h = plt.contourf(xx, yy, capacities_theoretical_anti_matched_mean, cmap='viridis', vmin=vmin, vmax=vmax,levels = np.arange(vmin, vmax, 0.1))
    # label the colorbar:
    cbar = plt.colorbar()
    cbar.set_label("Capacity", rotation=270, labelpad=20)
    plt.xlabel("Neuron activation heterogeneity")
    ...
    plt.savefig('results/Heterogeneous_network_capcity'+utils.make_name_with_time()+'.pdf')
